...
int get_input_line(prompt, insert, buf, if_cancelled, add_blank, do_echo)
...
short input_line(row, col, insert, buf, ch)
int row, col, ch;
char *insert, *buf;
{
...
short do_input_line(is_msg, row,col, prompt, insert, buf, if_cancelled, add_blank, do_echo, first_ch)
boolean is_msg;
int row, col;
char *prompt, *buf, *insert, *if_cancelled;
boolean add_blank, do_echo;
...
ŠK: 99 ‹à‰ò: 999999 ‘Ì—Í: 999(999) ‹­‚³: 99(99) Žç”õ: 99 ŒoŒ±: 21/10000000 ‹ó• 
0    5    1    5    2    5    3    5    4    5    5    5    6    5    7    5
*/

void print_stats(stat_mask)
register char stat_mask;
{
	char buf[16];
	boolean label;
	int row = DROWS - 1;
...
void sound_bell()
{
	putchar(7);
	fflush(stdout);
	MessageBeep(0xffffffff); /* windows */
}
