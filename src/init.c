...
extern char strdefname[30];

boolean init(argc, argv)
int argc;
char *argv[];
{
	char *pn;
...
*/	if (!nick_name[0])
	...
	strcpy(login_name, strdefname);
	DialogBox(hInst,MAKEINTRESOURCE(IDD_NAME),hWnd,(DLGPROC)DlgProc_Name);
...
char *estr;
{
	if (save_is_interactive) {
		...
		putstr(estr);
		putstr("\r\n");
		if(estr[0]) MessageBox(hWnd,estr,0,0); /* windows */
#if defined(JAPAN) && !defined(CURSES)
	...
	/* windows */
	saveINI(); /* Window関係の設定を保存するため、DestroyWindow前に行う。*/
...
void start_window()
{
	...
	if (*init_str)
		putstr(init_str);
	if (*init_str) MessageBox(hWnd,init_str,0,0); /* windows */
...
void stop_window()
{
	...
	if (*term_str)
		putstr(term_str);
	if (*term_str) MessageBox(hWnd,term_str,0,0); /* windows */
#endif
}
...
void env_get_value(s, e, add_blank, no_colon)
char **s, *e;
