...
void mv_monster(monster, row, col)
register object *monster;
short row, col;
{
	short i, n;
...
void move_mon_to(monster, row, col)
register object *monster;
register short row, col;
{
	short c;
...
boolean mon_can_go(monster, row, col)
register object *monster;
register short row, col;
{
	object *obj;
...
void wake_room(rn, entering, row, col)
short rn;
boolean entering;
short row, col;
{
...
boolean rogue_is_around(row, col)
short row, col;
{
	register short rdif, cdif;
...
boolean no_room_for_monster(rn)
int rn;
{
	register short i, j;
...
boolean mon_sees(monster, row, col)
object *monster;
short row, col;
{
	short rn, rdif, cdif;
	...
	return(boolean)((rdif >= -1)&&(rdif <= 1)&&(cdif >= -1)&&(cdif <= 1));
}
