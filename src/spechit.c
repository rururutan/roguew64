...
object *monster;
{
	...
	while (obj) {
		if ((obj->what_is == RING) &&
		    ...
		    (obj->in_use_flags & ON_EITHER_HAND) &&
		    (obj->is_cursed == 0)) {
		 	un_put_on(obj);
			goto adornment;
		}
...
boolean try_to_cough(row, col, obj)
short row, col;
object *obj;
{
...
void check_gold_seeker(monster)
object *monster;
{
	monster->m_flags &= (~SEEKS_GOLD);
}

boolean check_imitator(monster)
object *monster;
{
	char msg[80];
...
void get_closer(row, col, trow, tcol)
short *row, *col;
short trow, tcol;
{
	if (*row < trow) {
