...
unsigned short mask;
{
	...
	char *p;
#ifdef JAPAN
	char *msg = "  ÅÅÉXÉyÅ[ÉXÇâüÇµÇƒÇ≠ÇæÇ≥Ç¢ÅÅ";
	short len = 30;
#else
...
void make_scroll_titles()
{
	...
	for (i = 0; i < SCROLS; i++) {
		sylls = get_rand(2, 5);
#ifdef JAPAN
		(void) strcpy(id_scrolls[i].title, "Åu");
		len = 2;
		for (j = 0; j < sylls; j++) {
		...
		(void) strcpy(id_scrolls[i].title+(len-1), "Åv");
#else
...
void get_desc(obj, desc, capitalized)
register object *obj;
register char *desc;
boolean capitalized;
{
...
void inv_armor_weapon(is_weapon)
boolean is_weapon;
{
	if (is_weapon) {
		if (rogue.weapon) {
...
#ifdef JAPAN
static char *_num[10] = { "ÇO","ÇP","ÇQ","ÇR","ÇS","ÇT","ÇU","ÇV","ÇW","ÇX" };
