...
short one_move_rogue(dirch, pickup)
short dirch, pickup;
{
...
#ifndef ORIGINAL
		if (cur_room == PASSAGE && !blind && !confused && !strchr("yubn", dirch))
			bent_passage = 1;
#endif
		return(MOVE_FAILED);
	}
...
boolean next_to_something(drow, dcol)
register short drow, dcol;
{
...
boolean can_move(row1, col1, row2, col2) 
...
boolean check_hunger(messages_only)
