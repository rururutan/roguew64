...
/*	register short *bp, *tp;*/
	...
	if (screen_dirty) {/*
		...
		for(i=0,rev=1;i<DROWS;i++)
			if(lines_dirty[i])
			{
				// イタリック対応
				rc.left=0; rc.right=x*DCOLS;
				rc.top=y*i; rc.bottom=y*(i+1);
				FillRect(hDC,&rc,(HBRUSH)GetStockObject(BLACK_BRUSH));

				for(j=0;j<DCOLS;j++)
		{

			if(rev){ rev=0; SetBkColor(hDC,0x00000000);}

			switch((buffer[i][j]&0xFF00)>>8)
			{
			default:SetTextColor(hDC,0x00FFFFFF); break; // white
			case 1: SetTextColor(hDC,0x000000FF); break; // red
			case 2: SetTextColor(hDC,0x0000FF00); break; // green
			case 3: SetTextColor(hDC,0x0000FFFF); break; // yellow
			case 4: SetTextColor(hDC,0x00FF0000); break; // blue
			case 5: SetTextColor(hDC,0x00FF00FF); break; // magenta
			case 6: SetTextColor(hDC,0x00FFFF00); break; // cyan
			case 8:	SetTextColor(hDC,0x00000000); SetBkColor(hDC,0x00FFFFFF); rev=1; break; // RWHITE
			case 9:	SetTextColor(hDC,0x00FFFF00); SetBkColor(hDC,0x000000FF); rev=1; break; // RRED
			case 10:SetTextColor(hDC,0x00FF00FF); SetBkColor(hDC,0x0000FF00); rev=1; break; // RGREEN
			case 11:SetTextColor(hDC,0x00FF0000); SetBkColor(hDC,0x0000FFFF); rev=1; break; // RYELLOW
			case 12:SetTextColor(hDC,0x0000FFFF); SetBkColor(hDC,0x00FF0000); rev=1; break; // RBLUE
			case 13:SetTextColor(hDC,0x0000FF00); SetBkColor(hDC,0x00FF00FF); rev=1; break; // RMAGENTA
			case 14:SetTextColor(hDC,0x000000FF); SetBkColor(hDC,0x00FFFF00); rev=1; break; // RCYAN
			}

			/* SHIFT-JIS 漢字です */
			if(((buffer[i][j]&0x00FF)>=0x81 && (buffer[i][j]&0x00FF)<=0x9f)
			|| ((buffer[i][j]&0x00FF)>=0xe0 && (buffer[i][j]&0x00FF)<=0xfc))
			{
				// イタリック対応
				if(rev)
				{
				...
				two[0]=(char)buffer[i][j];
				two[1]=(char)buffer[i][j+1];
				two[2]=0;

				TextOut(hDC,j*x,i*y,two,2);
				j++;
			}
			else
			{
				...
				TextOut(hDC,j*x,i*y,(char*)&buffer[i][j],1);
			}
...
void refreshw()
{
	int i;
	screen_dirty = 1;
	for(i=0;i<DROWS;i++) lines_dirty[i]=1;
	refresh();
}
...
void clear()
{
	...
	InvalidateRect(hWnd,0,1); /* Windows */
}
...
#ifndef MSDOS
void tc_gtdata(tcf, fp, buf)	/* by Yasha */
